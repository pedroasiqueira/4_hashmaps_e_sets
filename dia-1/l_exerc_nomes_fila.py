...
#saída: ['giovanni', 'fernanda', 'rafa', 'fran', 'daniel', 'mirian', 'gabriel', 'matheus', 'yasmin']

# # SOLUÇÃO LENTA (quadrática): primeira a ser construída
# def get_order(orders):
#     orders_map = {person[0]: person[1] for person in orders}  # O(n)

#     next_in_line = ""
#     for person in orders_map:  # total do for: O(n²)
#         # Repete (n) vezes:
#         if person not in orders_map.values():  # O(n)
#             next_in_line = person  # O(1)

#     result = [next_in_line]
#     while next_in_line in orders_map:  # total do while: O(n)
#         # Repete (n) vezes:
